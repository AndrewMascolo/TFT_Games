...
Box OneP(&B), TwoP(&B), BT(&B), HostBtn(&B), GuestBtn(&B), YesBtn(&B), NoBtn(&B);
Circle BChip(&B), RChip(&B);
...
void SetupButtons()
{
  OneP.Coords(60, 70, 260, 100);
  OneP.Colors(LIGHT_BLUE, ROUNDED, FILL);
  OneP.Text("One Player", YELLOW, Big);

  TwoP.Coords(60, 130, 260, 160);
  TwoP.Colors(LIGHT_BLUE, ROUNDED, FILL);
  TwoP.Text("Two Players", YELLOW, Big);

  BT.Coords(60, 190, 260, 220);
  BT.Colors(LIGHT_BLUE, ROUNDED, FILL);
  BT.Text("Bluetooth", YELLOW, Big);

  HostBtn.Coords(120, 90, 200, 120);
  HostBtn.Colors(LIGHT_BLUE, ROUNDED, FILL);
  HostBtn.Text("Host", YELLOW, Big);

  GuestBtn.Coords(120, 150, 200, 180);
  GuestBtn.Colors(LIGHT_BLUE, ROUNDED, FILL);
  GuestBtn.Text("Guest", YELLOW, Big);

  BChip.Coords(100, 130, 30);
  BChip.Colors(BLACK, FILL);
  BChip.Padding(2, WHITE);

  RChip.Coords(220, 130, 30);
  RChip.Colors(RED, FILL);
  RChip.Padding(2, WHITE);
  
  YesBtn.Coords(80, 159, 145, 189);
  YesBtn.Colors(GREEN, ROUNDED, FILL);
  YesBtn.Text("YES", BLACK, Big);

  NoBtn.Coords(170, 159, 235, 189);
  NoBtn.Colors(RED, ROUNDED, FILL);
...
void set_Gamemode()
{
  ...
  while ( true )
  {
    ...
    else if (TwoP.Delay(200))
    {
      gameMode = 1;
      break;
    }
    else if (BT.Delay(200))
    {
      gameMode = 2;
      SetHost();
      break;
...
void SetHost()
{
  ...
  while ( true )
  {
    if (HostBtn.Delay(200))
    {
      Player = 0;
      Host = !Player;
      break;
    }
    else if (GuestBtn.Delay(200))
    {
      Player = 1;
      Host = !Player;
      break;
...
void PlayPerson()//
{
  ...
  {
    if (TouchButton(45 + col * 25, 50, 72 + col * 25, 195))
    {
      newRow = CheckMap(col);
      if (newRow >= 0)
      {
...
void PlayComputer()//
{
  ...
  {
    if ( Player ? col = SmartAIResponse() : TouchButton(45 + (col - 1) * 25, 50, 72 + (col - 1) * 25, 195)) // Get col
    {
      if (!MapFull[col - 1])
      {
        Remaining--;
...
bool TouchButton(int x, int y, int x2, int y2)
{
  if (myTouch.dataAvailable())
  {
    ...
    return (IsWithin(xc, x, x2) && IsWithin(yc, y, y2));
  }
  else
    return false;
}
